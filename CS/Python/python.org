...
- count number of value bigger than a certain value:
#+BEGIN_SRC python
left = [1,0,1]
reduce(lambda acc, x: acc + (1 if x > 0 else 0), left, 0)
#+END_SRC

- get all value bigger than a certain value:
#+BEGIN_SRC python
left = [1, 0, 1]
[i for i in left if i > 0]
#+END_SRC
...
convert one column into two columns, property and value.
#+BEGIN_SRC python
"""
Parameters:
frame : DataFrame
